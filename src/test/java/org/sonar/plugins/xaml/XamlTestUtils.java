...
import org.sonar.plugins.xaml.utils.XamlUtils;

public class XamlTestUtils{
  ...
  public static File loadResource(String resourceName) {
    URL resource = XamlTestUtils.class.getResource(resourceName);
  ...
  public static Project mockProject() {
    File baseDir;
    baseDir = loadResource("/org/sonar/plugins/xaml/");  //we skip "SampleProject" dir because report dirs as here
    
    List<File> sourceDirs = new ArrayList<File>();
    sourceDirs.add(loadResource("/org/sonar/plugins/xaml/SampleProject/") );
  ...
  public static Project mockProject(File baseDir, List<File> sourceDirs) {
    ...
    when(fileSystem.getSourceFiles(mockXamlLanguage())).thenReturn(mainSourceFiles);
    when(fileSystem.mainFiles(XamlLanguage.KEY)).thenReturn(mainFiles);
    when(fileSystem.getSourceDirs()).thenReturn(sourceDirs);


    Project project = mock(Project.class);
    when(project.getFileSystem()).thenReturn(fileSystem);
    XamlLanguage lang = mockXamlLanguage();
  ...
  public static XamlLanguage mockXamlLanguage(){
    return new XamlLanguage(mock(Configuration.class));
  }
  
  private static List<File> scanForSourceFiles(List<File> sourceDirs) {
    List<File> result = new ArrayList<File>();
    String[] suffixes = mockXamlLanguage().getFileSuffixes();
